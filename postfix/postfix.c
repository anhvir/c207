/* Anh Vo, avo@unimelb.edu.au, for COMP20007 2020 S1 @ unimelb
  ...
  APPROACH:  Here, we use stack to evaluate the expression, for example,
   ...
   IMPORTANT NOTES:
	This program should be used with your implementation of stack_adt
	Supposing:
		- stack.h is the interface of your stack module
		- the stack data type is stack_adt, and there are functions: 
			stack_adt *new_stack()
			void stack_free(stack_adt *s)
...
int main(int argc, char *argv[]) {
	...
	while (fgets(s, MAX_STRLEN, stdin)) { // read new expression to s
		...
		for (p=s; *p; p++) {
			...
			b= pop(st);    // now takes 2 operands & perform calculation
			a= pop(st);
			switch (*p) {
				case '+': c= a+b; break;
				case '-': c= a-b; break;
